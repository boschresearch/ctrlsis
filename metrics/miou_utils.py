...
def intersectionAndUnion_pytorch(imPred, imLab, numClass): 

    imPred = imPred.clone() + 1
    imLab = imLab.clone() + 1
    # Remove classes from unlabeled pixels in gt image.
    # We should not penalize detections in unlabeled portions of the image.
    imPred = imPred * (imLab > 0)
    ...
    # Compute area intersection:
    intersection = imPred * (imPred == imLab)
    (area_intersection, _) = torch.histogram(
        intersection, bins=numClass, range=(1, numClass))

    # Compute area union:
    (area_pred, _) = torch.histogram(imPred, bins=numClass, range=(1, numClass))
    (area_lab, _) = torch.histogram(imLab, bins=numClass, range=(1, numClass))
    area_union = area_pred + area_lab - area_intersection
