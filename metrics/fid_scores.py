...
from metrics.fid_folder.inception import InceptionV3
import matplotlib.pyplot as plt
from utilfunctions import preprocess_oasis_input
from torchvision.utils import save_image 

class fid_pytorch:
    def __init__(self, opt, dataloader_val, sanity_check_folder=None):
        self.opt = opt
        self.dims = 2048
        ...
        self.sanity_check_folder = sanity_check_folder

        m1_path = f"metrics/precomputed/{opt.dataset}_m1.pth"
        s1_path = f"metrics/precomputed/{opt.dataset}_s1.pth"
        if os.path.exists(s1_path):
            print("Loaded pre-computed statistics from metrics/precomputed/")
            self.m1, self.s1 = torch.load(m1_path), torch.load(s1_path)
        else:
            self.m1, self.s1 = self.compute_statistics_of_val_path(dataloader_val)
            torch.save(self.m1, m1_path)
            torch.save(self.s1, s1_path)
            print("Saved pre-computed statistics from metrics/precomputed/")
    ...
    def compute_fid_with_valid_path(self, generator):
        pool = []
        self.model_inc.eval()

        with torch.no_grad():
            for i, data_i in enumerate(self.val_dataloader):
                _, label = preprocess_oasis_input(self.opt, data_i) 
                ...
                if i <= 2 and self.sanity_check_folder is not None:
                    save_path = os.path.join(self.sanity_check_folder, f"synth_{i}.jpg")
                    save_image(generated, save_path)
                    print(f"saved /fs/scratch/rng_cr_bcai_dl/esc2rng/github_enterprise/editoasis/{save_path}")
    ...
    def numpy_calculate_frechet_distance(self, mu1, sigma1, mu2, sigma2, eps=1e-6):
        ...
        mu1, sigma1, mu2, sigma2 = (
            mu1.detach().cpu().numpy(),
            sigma1.detach().cpu().numpy(),
            mu2.detach().cpu().numpy(),
            sigma2.detach().cpu().numpy(),
        )
    ...
    def update(self, generator, cur_iter):
        print("--- Iter %s: computing FID ---" % (cur_iter))
        cur_fid = self.compute_fid_with_valid_path(generator)
    ...
    def update_logs(self, cur_fid, epoch):
        ...
        plt.grid(b=True, which="major", color="
        plt.minorticks_on()
