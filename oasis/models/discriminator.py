...
class OASIS_Discriminator(nn.Module):
    def __init__(self, opt):
        ...
        self.channels = [opt.img_channels, 128, 128, 256, 256, 512, 512]
        ...
        if opt.lambda_bneck > .0:
            norm_layer = norms.get_spectral_norm(opt)
            self.bneck_linear = norm_layer(nn.Linear(self.channels[-1], 1))
    ...
    def forward(self, input, return_bottleneck=False):
        ...
        for i in range(1, len(self.body_down)):
            ...
            #     x = interpolate(x, encoder_res[-i-1].shape[-2:], mode='bilinear', align_corners=False)
            x = self.body_up[i](torch.cat((encoder_res[-i-1], x), dim=1))

        ans = self.layer_up_last(x)

        if return_bottleneck and self.opt.lambda_bneck > .0:
            bottleneck = F.leaky_relu(encoder_res[-1], 0.2)
            bottleneck = torch.sum(bottleneck, (2, 3))
            bottleneck = self.bneck_linear(bottleneck)

            return ans, bottleneck
        else:
            return ans
